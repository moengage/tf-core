resource "aws_subnet" "private" {
  ...
  tags = merge(
    local.default_tags,
    {
      "Name" = "${local.resource_identifier}-private-${substr(element(var.availability_zones, count.index), -2, -1)}"
    },
    {
...
resource "aws_subnet" "public" {
  ...
  tags = merge(
    local.default_tags,
    {
      "Name" = "${local.resource_identifier}-public-${substr(element(var.availability_zones, count.index), -2, -1)}"
    },
    {
      "ResourceType" = "public"
    },
