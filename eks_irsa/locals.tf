locals {
  ...
  resource_identifier = "${lower(var.environment)}-${lower(data.aws_region.current.name)}-${lower(var.eks_cluster_name)}-${lower(var.kubernetes_service_account_namespace)}-${lower(var.kubernetes_service_account_name)}"
