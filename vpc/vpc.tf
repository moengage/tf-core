...
resource "aws_nat_gateway" "nat_gw" {
  ...
  tags = "${merge(
    local.default_tags,
    map("Name", "${local.resource_identifier}-vpc-nat-gw-${substr(element(var.availability_zones, count.index), -2, -1)}"),
    map("AvaiabilityZone", "${element(var.availability_zones, count.index)}"),
  )}"
