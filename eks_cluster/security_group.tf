resource "aws_security_group" "default" {
  ...
  tags = merge(
    ...
    var.extra_tags,
    {
      "kubernetes.io/cluster/${local.resource_identifier}" = "owned"
    },
  )
}
...
resource "aws_security_group_rule" "default_ingress" {
  ...
  source_security_group_id = aws_security_group.worker_default[0].id
...
resource "aws_security_group_rule" "worker_default_ingress" {
  ...
  to_port                  = 443
  protocol                 = "tcp"
  security_group_id        = aws_security_group.worker_default[0].id
