locals {
  ...
  lb_security_groups = "${split(",", var.lb_create_security_group ? join(",",  concat(compact(aws_security_group.alb.*.id), var.lb_security_groups)) : join(",", var.lb_security_groups))}"
}
