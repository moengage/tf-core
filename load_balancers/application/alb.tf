resource "aws_lb" "default" {
  name                             = "${substr("${local.resource_identifier}", 0, min(28,  length("${local.resource_identifier}")))}-alb"
  ...
  tags = "${merge(
    local.default_tags,
    map("Name", "${local.resource_identifier}")
  )}"
}

resource "aws_lb_target_group" "default" {
  name     = "${substr("${local.resource_identifier}", 0, min(25, length("${local.resource_identifier}")))}-def-tg"
  port     = 80
  protocol = "HTTP"
  vpc_id   = "${var.vpc_id}"
  ...
  tags = "${merge(
    local.default_tags,
    map("Name", "${local.resource_identifier}-def-tg"),
  )}"
}
